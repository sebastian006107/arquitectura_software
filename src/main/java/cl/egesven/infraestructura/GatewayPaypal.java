...
public class GatewayPaypal {
    ...
    public Pago procesarPago(int idPedido, int idMedioPago, BigDecimal monto) {
        ...
        String token = "PP-" + System.currentTimeMillis() + "-" + random.nextInt(10000);

        System.out.println("[SIMULACION PayPal] Procesando pago por $" + monto + "...");
        System.out.println("[SIMULACION PayPal] Token: " + token);
        ...
        if (aprobado) {
            System.out.println("[SIMULACION PayPal] Pago APROBADO");
        } else {
            System.out.println("[SIMULACION PayPal] Pago RECHAZADO");
        }
